...
def process_investigative_query(request: IntelligenceQuery):
    ...
    try:
        ...
        # PATTERN / PREDICT / NETWORK → Track 4 Mocked Analytic Logic
        if intent == "PATTERN":
            print(f"🗺️ [ROUTER] {intent} → executing geospatial clustering logic")
            route_payload = {
                ...
                "coordinates": [
                    ...
                    {"lat": 12.9279, "lng": 77.6271, "intensity": 0.9}
                ],
                "officer_id": request.officer_id,
                "raw_query": request.query,
                "translated_query": search_query_en,
                "timestamp": datetime.datetime.now().isoformat()
            }
            answer_text_en = "Geospatial clustering complete. Showing crime hotspots based on recent FIR data."

        elif intent == "NETWORK":
            print(f"🕸️ [ROUTER] {intent} → executing network analysis logic")
            route_payload = {
                "target_module": intent,
                "type": "graph",
                "nodes": [
                    {"id": "SUS-1001", "label": "Raja Kumar", "group": "Suspect"},
                    {"id": "SUS-1002", "label": "Ali Khan", "group": "Suspect"},
                    {"id": "FIR-2023", "label": "Robbery Case", "group": "Incident"}
                ],
                "edges": [
                    {"source": "SUS-1001", "target": "FIR-2023"},
                    {"source": "SUS-1002", "target": "FIR-2023"},
                    {"source": "SUS-1001", "target": "SUS-1002"}
                ],
                "officer_id": request.officer_id,
                "raw_query": request.query,
                "translated_query": search_query_en,
            ...
            answer_text_en = "Network analysis complete. Displaying suspect associations and linked incidents."

        elif intent == "PREDICT":
            print(f"📈 [ROUTER] {intent} → executing forecasting logic")
            route_payload = {
                "target_module": intent,
                "type": "forecast",
                "timeline": [
                    {"date": "2023-10-01", "predicted_crimes": 12},
                    {"date": "2023-10-02", "predicted_crimes": 15},
                    {"date": "2023-10-03", "predicted_crimes": 9}
                ],
                "risk_areas": ["Bengaluru South", "Whitefield"],
                "officer_id": request.officer_id,
                "raw_query": request.query,
                "translated_query": search_query_en,
                "timestamp": datetime.datetime.now().isoformat()
            }
            answer_text_en = "Forecasting complete. Showing predictive crime trends for the upcoming week."
